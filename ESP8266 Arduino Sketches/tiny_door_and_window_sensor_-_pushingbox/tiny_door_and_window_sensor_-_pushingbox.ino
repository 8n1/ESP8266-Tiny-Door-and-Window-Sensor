...
const char* pushingbox_device_id = "pushingbox_device_id";
...
int send_http_request() {
  ...
  String request_url = "/pushingbox?devid=" + String(pushingbox_device_id);
...
void setup()
{
  ...
  query_string = "&vbat=" + String(vbat);
  
  // RSSI
  int rssi = 2 * (WiFi.RSSI() + 100);
  query_string += "&rssi=" + String(rssi);
  
  // SWITCH STATE
  int switch_state = digitalRead(SWITCH_PIN);
  query_string += "&state=" + String(switch_state);
  ...
  query_string += "&uptime=" + String(uptime_millis);
